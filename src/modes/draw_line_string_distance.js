 ...
 */
DrawLineStringDistance.updateExtendedGuidelinesOpacity = function (state) {
  const map = this.map;
    ...
    if (!snapInfo) {
      ...
      if (snapping && snapping.snappedFeature) {
        ...
        if (isExtendedGuideline) {
          ...
          // Snapping to something else - check if it's a snappingPoint or line that intersects with extended guideline
          const tempSnapInfo = this.getSnapInfo(lngLat);
