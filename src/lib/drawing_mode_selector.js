...
const SUB_MODE_ORDER = [
  DRAWING_SUB_MODES.FREE,
  DRAWING_SUB_MODES.RECTANGLE,
  DRAWING_SUB_MODES.LINE,
];
...
export function createDrawingModeSelector(ctx, state, options = {}) {
  ...
  modeLabel.innerHTML = `<span class="key">M</span><span class="text">${
    SUB_MODE_LABELS[state.drawingSubMode]
  }</span>`;
  ...
  const fireSubModeChange = () => {
    if (ctx.options.onDrawingSubModeChange) {
      ctx.options.onDrawingSubModeChange(
        state.drawingSubMode,
        SUB_MODE_LABELS[state.drawingSubMode],
        modeLabel
      );
    }
  ...
  const keyHandler = (e) => {
    if (e.key === "m" || e.key === "M") {
      ...
      state.drawingSubMode =
        SUB_MODE_ORDER[(currentIndex + 1) % SUB_MODE_ORDER.length];
  ...
  // Call the callbacks with initial mode
  if (onModeChange) {
    onModeChange(state.drawingSubMode);
  }
  fireSubModeChange();
...
export function removeDrawingModeSelector(state) {
  ...
  if (state.modeSelectorClickHandler && state.modeSelectorLabel) {
    state.modeSelectorLabel.removeEventListener(
      "click",
      state.modeSelectorClickHandler
    );
    state.modeSelectorClickHandler = null;
  }
  if (state.modeSelectorContainer && state.modeSelectorContainer.parentNode) {
    state.modeSelectorContainer.parentNode.removeChild(
      state.modeSelectorContainer
    );
